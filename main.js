...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [SILENT HACKER-MD] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
            ...
            if (connection === 'open') {
                ...
                if (!existingSession) {
                    await conn.sendMessage(userJid, {
                        ...
                        caption: `\n╭────────────────────◇\n│✦ *𝐒𝐈𝐋𝐄𝐍𝐓 𝐇𝐀𝐂𝐊𝐄𝐑 𝐌𝐃 — CONNECTED* 🔥\n│✦ Type *${prefix}menu* to see all commands 💫\n│✦ Prefix 『 ${prefix} 』  Mode 〔${mode}〕\n╰────────────────────○\n*© Powered by 𝐒𝐈𝐋𝐄𝐍𝐓 𝐇𝐀𝐂𝐊𝐄𝐑*`
                    });
                }
            }
            if (connection === 'close') {
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = ['120363405630606308@newsletter'];
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© SILENT HACKER MD',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN: SILENT HACKER MD BOY\nORG: SILENT HACKER MD BOY;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '©𝐒𝐈𝐋𝐄𝐍𝐓 𝐇𝐀𝐂𝐊𝐄𝐑 𝐌𝐃' } }
                    }},
        ...
        arslanLog(`𝐒𝐈𝐋𝐄𝐍𝐓 𝐇𝐀𝐂𝐊𝐄𝐑 Pair error: ${err.message}`, 'error');
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: '𝐬𝐢𝐥𝐧𝐭 𝐇𝐚𝐜𝐤𝐞𝐫 𝐌𝐝  is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 𝐒𝐈𝐋𝐄𝐍𝐓 𝐇𝐀𝐂𝐊𝐄𝐑 𝐌𝐃 — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
